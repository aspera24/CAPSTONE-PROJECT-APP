...
// I/O Pins
#define MOTOR_RELAY 17     //Feed motor relay
...
float calibration_factor = -123822;  // Fixed calibration
...
void loop() {
  ...
  if (Firebase.ready() && signupOK && (nowMillis - lastCheck >= CHECK_INTERVAL)) {
    ...
    int percentage = constrain(map(distance, 40, 10, 0, 100), 0, 100);
    String level = getLevelStatus(percentage);

    Serial.printf("\nDistance: %d cm\nPercentage: %d%%\nLevel: %s\n", distance, percentage, level.c_str());

    Firebase.RTDB.setInt(&fbdo, "sensor/distance_cm", distance);
    Firebase.RTDB.setInt(&fbdo, "sensor/percentage", percentage);
    Firebase.RTDB.setString(&fbdo, "sensor/level", level);
...
void startFeedingToWeight(float targetWeight, String mode) {
  ...
  while (true) {
    ...
    if (currentWeight >= targetWeight - 0.05) {
...
String getLevelStatus(int percentage) {
  if (percentage >= 0 && percentage <= 25) return "VERY LOW";
  if (percentage > 25 && percentage <= 50) return "LOW";
  if (percentage > 50 && percentage <= 75) return "MED";
  return "HIGH";  // for 76–100
}
